...
(function() {
	...
	app.addController("updateCrowdFundController", updateCrowdFundController);
	updateCrowdFundController.$inject = ['$scope', '$rootScope', '$window', 'crowdFundService', 'publicService', 'addressService', '$state', '$filter', '$stateParams', '$timeout'];

	function updateCrowdFundController($scope, $rootScope, $window, crowdFundService, publicService, addressService, $state, $filter, $stateParams, $timeout) {
		...
		vm.getRepayInfo = getRepayInfoFn; //获取单一回报详情
				...
				crowdFundService.detail(params).success(function(data) {
					if (data.ret != 200) {
						...
						if (data.data.code == 20000) {
							...
							//TODO 赋值
							//vm.submitForm.welfare_id = vm.editForm.welfare_id;
							vm.submitForm.title = vm.editForm.name;
							vm.submitForm.brief = vm.editForm.brief;
							//vm.submitForm.address.province = vm.editForm.province;
							//vm.submitForm.address.city = vm.editForm.city;
							vm.submitForm.address.location = vm.editForm.address;
							vm.submitForm.limit_price = parseFloat(vm.editForm.limit_price);
							vm.submitForm.deal_days = $filter('toTime')(vm.editForm.deal_days);
							vm.submitForm.tags_match = vm.editForm.tags_match;
							vm.submitForm.image = vm.editForm.image;
							vm.submitForm.vedio = vm.editForm.vedio;
							vm.submitForm.description = vm.editForm.description;
							//vm.submitForm.item_id = vm.editForm.item_id.split(",");
							...
							addressService.getProvinceList(function(provinceArray) {
								...
								vm.getCityList(vm.editForm.province, "cityArray", function() {
									vm.submitForm.address.city = vm.editForm.city; //城市赋值，运行之后，城市select会被选中
								});

							});

							//获取众筹类型select
							vm.getCrowdFundTypeList(function() {
								vm.submitForm.welfare_id = vm.editForm.welfare_id;
								//下面这个for循环里的 vm.crowdFundTypeArray 是先通过接口请求赋值了数据的！

								for (var i = 0; i < vm.crowdFundTypeArray.length; i++) {
									if (vm.submitForm.welfare_id == vm.crowdFundTypeArray[i].id) {
										vm.submitForm.welfare_name = vm.crowdFundTypeArray[i].name;
									}
								}
							});

							//获取单一回报详情
							var repayArray = vm.editForm.item_id.split(',');
							for (var i = 0; i < repayArray.length; i++) {
								if (repayArray[i] == 0) {
									//这里等于 0，说明是无回报
									vm.submitForm.hasRepay = '0';
									return false;
								} else {
									vm.submitForm.hasRepay = '1';
									vm.getRepayInfo(repayArray[i]);
								}
		...
		//
		//
		function getCrowdFundTypeListFn(callback) {
			crowdFundService.getCrowdFundTypeList().success(function(data) {
				if (data.ret != 200) {
					...
					if (data.data.code == 20000) {
						...
						//如果是编辑，则此处会给之前选中的众筹类型赋值，使html被选中
						if (callback) {
							callback();
						}
					} else {
		...
		//
		//
		function getRepayInfoFn(id) {
			var params = {
				"item_id": id
			}
			crowdFundService.getRepayInfo(params).success(function(data) {
				if (data.ret != 200) {
					window.showAutoDialog(data.msg);
					return false;
				} else {
		...
		function submitFn() {
			...
			var params = {
				"deal_id": $stateParams.id, //传了这个deal_id，就变在编辑
